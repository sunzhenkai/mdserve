...
将 HTML 文件放入文档目录即可，文件树会自动收录。点击文件后在应用壳层（侧栏、大纲、工具栏）内打开。内容右上角工具栏提供「打开独立展示页」，会在新标签页以 `/api/asset/<文档路径>` 原页展示（不带 mdserve 壳层）；使用路径形式以便页内相对图片、脚本、样式按文档所在目录解析。
...
- 外链（`http(s)://`）在新标签页打开
- Markdown 正文里的 `.html` 链接仍直接打开 `/api/asset/<文档路径>` 整页（与目录树的 iframe 预览是两条路径）
